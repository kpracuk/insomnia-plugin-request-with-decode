...
module.exports.templateTags = [
  {
    ...
    args: [
      ...
      {
        displayName: 'Run decodeURIComponent() on returned value?',
        hide: args => args[0].value !== 'cookie',
        type: 'enum',
        options: [
          {
            displayName: 'No',
            value: 'no',
            description: 'Leave value as is',
          },
          {
            displayName: 'Yes',
            value: 'yes',
            description: 'Pass value through decodeURIComponent()',
          }
        ],
      },
    ],

    async run(context, attribute, name, folderIndex, decode) {
      const { meta } = context;
      ...
      switch (attribute) {
        ...
        case 'cookie':
          ...
          if(decode === 'yes'){
            return decodeURIComponent(await getCookieValue(cookieJar, url, name));
          }
